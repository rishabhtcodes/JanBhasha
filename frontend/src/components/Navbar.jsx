...
import { Languages, LayoutDashboard, History, BookOpen, Shield, LogOut, User, Menu, X, Sparkles } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 glass-panel border-b border-slate-800/80 bg-slate-950/80 backdrop-blur-md">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div>
              <span className="text-xl font-extrabold tracking-tight text-white group-hover:text-indigo-400 transition-colors">
                Jan<span className="gradient-text">Bhasha</span>
              </span>
            ...
            {user ? (
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center text-xs font-bold text-white uppercase">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 rounded-lg shadow-md shadow-indigo-500/20 transition-all hover:scale-105"
                >
                  Get Started Free
                </button>
              </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden glass-panel border-b border-slate-800 px-4 pt-2 pb-6 space-y-2">
          <Link to="/translate" onClick={() => setMobileMenuOpen(false)} className="block px-3 py-2 text-slate-300 hover:bg-slate-800 rounded-md">Translate Engine</Link>
          {user ? (
            <>
            ...
            <div className="pt-4 border-t border-slate-800 space-y-2">
              ...
              <button onClick={() => { onOpenAuth('register'); setMobileMenuOpen(false); }} className="w-full text-center px-4 py-2 bg-indigo-600 text-white rounded-lg">Get Started</button>
            </div>
          )}
        </div>
      )}
